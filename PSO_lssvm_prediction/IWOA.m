...
class OmniglotBuilder:
    ...
    def build_experiment(self, batch_size, num_channels, lr, image_size, classes_per_set, samples_per_class, keep_prob, fce, optim, weight_decay, use_cuda):
    ...
    def run_val_epoch(self, total_val_batches):
        ...
        with tqdm.tqdm(total=total_val_batches) as pbar:
            for i in range(total_val_batches):
                ...
                batch_size = y_support_set.size()[0]
                y_support_set_one_hot = Variable(
                    torch.zeros(batch_size, sequence_length, self.classes_per_set).scatter_(2,y_support_set.data,1), requires_grad=False)
